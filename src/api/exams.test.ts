...
const wireExam = {
  id: 'EX1', name: 'Term 1', type: 'Term', grades: 'VI-XII',
  from_date: '2026-09-08', to_date: '2026-09-20', subject_count: 6,
  status: 'scheduled', marks_entered_pct: 0, published: false,
}
...
describe('listExams', () => {
  it('maps marks_entered_pct -> marksEntered', async () => {
    ...
    expect(rows[0]).toMatchObject({ id: 'EX1', name: 'Term 1', from: '2026-09-08', to: '2026-09-20', subjects: 6, marksEntered: 0, published: false })
  })
})
...
describe('createExam', () => {
  it('POSTs /exams with marksEntered -> marks_entered_pct', async () => {
    ...
    expect(body.subject_count).toBe(6)
    expect(body.from_date).toBe('2026-09-08')
    expect(body.to_date).toBe('2026-09-20')
    expect(body.name).toBe('Term 1')
    expect(body.marks_entered_pct).toBeUndefined()
