...
export function useCreateFeeHead(): UseMutationResult<FeeHead, Error, { name: string; code?: string; isTransportFeeHead?: boolean; description?: string }> {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (input: { name: string; code?: string; isTransportFeeHead?: boolean; description?: string }) => createFeeHead(input),
    onSuccess: () => { qc.invalidateQueries({ queryKey: queryKeys.feeHeads.all }) },
  })
}

export function useUpdateFeeHead(): UseMutationResult<FeeHead, Error, { id: string; patch: Partial<Pick<FeeHead, 'name' | 'code' | 'active' | 'isTransportFeeHead' | 'description'>> }> {
  const qc = useQueryClient()
  return useMutation({
